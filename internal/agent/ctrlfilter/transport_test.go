...
// (New's Rewrite). The controller keeps a small request-header buffer on WRITES
// and rejects (400 + RST) a POST whose headers exceed it; a LAN-direct browser
// write fits, but proxy-forwarding headers an upstream adds (X-Forwarded-*,
// X-Real-Ip, Forwarded, Via) push it over. Verified live 2026-08-29 on a
// ProCon.IP: the same usrcfg.cgi POST is 400 with them and 200 without. So the
// proxy must strip every one before the write reaches the controller — while
// leaving the caller's own headers (the controller's Basic Auth) untouched.
func TestForwardingHeadersNeverReachTheController(t *testing.T) {
	...
	// X-Forwarded-Port is a NON-standard forwarding header the stdlib does not
	// pre-delete — only New's prefix sweep removes it, so it guards that too.
	stripped := []string{
